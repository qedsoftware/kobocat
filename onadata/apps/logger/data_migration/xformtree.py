...
class XFormTree(object):
    ...
    def get_groups(self):
        instance = self.get_head_instance()
        return list(filter(self.is_group, instance))
    ...
    def get_structured_fields_as_tree(self):
        return compose(Tree.construct_tree, self.get_structured_fields)()
    ...
    def remained_fields(self, other_tree):
        """Return fields that remained in tree compared to other tree"""
        return list(
            set(self.get_fields()) - set(self.added_fields(other_tree))
        )
